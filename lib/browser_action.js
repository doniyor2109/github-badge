...
import { badgeUrlFromPattern, staticBadgeUrl } from 'shields.io/core/badge-urls/make-badge-url'
...
import { getStore, setStore, onStorageChange } from './store';
import { flattenedServices, categories } from './data';
import { baseUrl } from './constants';
import { addBadgeMessage, removeBadgeMessage, REPO_INFO } from './message';
import { getGithubTabs, sendMessageToTab, onExtensionMessage } from './helpers';
...
function App() {
  ...
  useEffect(() => {
    ...
    getStore('lastRepoInfo', {}, (repoInfo) => {
    ...
    onStorageChange(function(changes) {
      if (changes.lastRepoInfo) {
        setRepoInfo(changes.lastRepoInfo.newValue);
      }
    });
  }, []);
  ...
  function renderItem(example) {
    const { preview, example: { pattern } } = example
    const { label, message, color, style, namedLogo } = preview

    const previewUrl = repoInfo ?
      badgeUrlFromPattern({
        baseUrl,
        pattern,
